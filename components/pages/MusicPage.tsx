  ...
  return (
    <main id="main">
      ...
      <section className="section pt-12 md:pt-16">
        <Stain shape="clay" drift="b" className="right-0 top-6 h-[28rem] w-[34rem]" opacity={0.26} />
        <Stain shape="sage" drift="a" className="bottom-0 left-0 h-[24rem] w-[28rem]" opacity={0.2} />
        <div className="container">
          <h1 className="text-display font-semibold reveal">{t("music.heading")}</h1>
          <p className="measure mt-5 text-h3 text-ink-soft reveal">{t("music.intro")}</p>
          <ul className="mt-14 grid gap-12 md:grid-cols-2 md:gap-x-12 md:gap-y-16">
            {releases.map((release) => (
              <li key={release.slug} className="grid gap-6 sm:grid-cols-12 reveal">
      ...
      <section className="section pt-0">
        <div className="container">
          <h2 className="text-h2 font-semibold reveal">{t("music.appearsOn")}</h2>
          <ul className="mt-8 max-w-3xl">
            {site.appearsOn.map((item) => (
              <li key={item.url} className="grid gap-1 border-b border-line py-4 sm:grid-cols-[4rem_1fr] sm:gap-6 reveal">
