...
Round 7 answered "still too subtle" on the background and reworked the hero photograph. Each mark now carries two animations on periods that never line up, a closed path with non-uniform scale so its silhouette changes rather than slides, and a density that breathes; sections carry four or five overlapping marks, because the moving overlap is what reads as smoke. Measured at fifteen to thirty pixels of travel and thirty to eighty of billow in three seconds. The hero photograph is mounted and printed in a duotone built from the clay pigment, ten percent smaller, at its own proportion, and the blue LED strip across its top was removed at the source with a new `cropTop` option rather than hidden in CSS.
...
- **Printed photographs**: `.sleeve-print` in `app/globals.css` mounts a photograph on paper and prints it in a duotone: grayscale, then a `lighten` layer sets the black point to `--color-print-ink` and a `darken` layer caps the white point at `--color-print-paper`. Both are the clay pigment at new lightness, so the palette stays closed. Used in the hero only; the gallery keeps the document untouched.
