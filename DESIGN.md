...
Marks of the artist's own paint, lifted off the sleeve by `scripts/pigments.mjs`: the paper is dissolved away by turning the paint's own density into an alpha channel, and every edge and corner is faded so a crop can never leave a rectangle. The file carries only the shape; the colour is a token, which is what lets a release page stain the page in its own record's pigment. They sit at 0.16 to 0.34 opacity, two or three per section, always inside their section, always behind the content, always `aria-hidden`, and they disappear entirely under forced colours or where masks are unsupported. Each mark carries two animations on periods that never line up: a closed path it travels (22s, 30s, 26s, linear, looping) and a density that breathes (17s, 19s, 23s). The path scales non-uniformly, so a mark stretches one way while it compresses the other and its silhouette actually changes rather than sliding. `--stain-phase` offsets a mark inside its lane so two marks sharing a lane do not move as one body, and sections carry four or five overlapping marks, because what reads as smoke is the moving overlap and not any single shape. Measured: fifteen to thirty pixels of travel and thirty to eighty pixels of billow in three seconds, against half a pixel when the paths ran 96 to 138 seconds on an eased alternation and the page read as a still picture. Transform and opacity only, so no frame costs a layout, and everything stops under reduced motion.
